...
if(isset($_POST['mesa']) && isset($_POST['mesero']) && isset($_POST['referencia'])){
	if($mesa=="" || $mesero=="" || $ref==""){		
		...
		$mysqli->query("insert into referencia values(0,'$mesa','$mesero','$ref','')")or die("ERROR: " .$mysqli->error); 
		$mysqli->query("insert into orden values(0,".$mysqli->insert_id.",CURDATE())")or die("ERROR: " .$mysqli->error);
